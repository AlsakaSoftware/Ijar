...
struct GroupPickerSheet: View {
    ...
    let propertyGroupService: PropertyGroupService
    ...
    var body: some View {
        NavigationStack {
            ...
            .task {
                ...
                let currentGroups = await propertyGroupService.getGroupsForProperty(propertyId: property.id)
    ...
    private func createGroup() {
        Task {
            if let group = await propertyGroupService.createGroup(name: newGroupName) {
    ...
    private func saveChangesAndDismiss() {
        ...
        Task {
            ...
            for groupId in toAdd {
               _ = await propertyGroupService.addPropertyToGroup(propertyId: property.id, groupId: groupId)
            }

            // Remove from unselected groups
            for groupId in toRemove {
                _ = await propertyGroupService.removePropertyFromGroup(propertyId: property.id, groupId: groupId)
            }
