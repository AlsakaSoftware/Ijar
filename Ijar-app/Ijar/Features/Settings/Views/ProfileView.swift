    ...
    var body: some View {
        ...
        .alert("Delete Account", isPresented: $viewModel.showingDeleteConfirmation) {
            ...
            Button("Delete", role: .destructive) {
                Task {
                    await viewModel.deleteAccount(authService: authService)
                }
            }
        } message: {
    ...
    private var authenticatedContent: some View {
        VStack(spacing: 30) {
            ...
            if !subscriptionManager.isSubscribed {
                PremiumUpgradeBanner(showPaywall: $viewModel.showingPaywall)
                    .padding(.horizontal)
                    .padding(.top, 20)
            }
            ...
            VStack(spacing: 16) {
                ...
                Button(action: {
                    Task {
                        await viewModel.signOut(authService: authService)
                    }
                }) {
                    HStack {
