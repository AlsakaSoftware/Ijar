-- Apply complete schema including all missing tables and views
-- This ensures property_feed view and all required tables exist
...
-- User search queries (with postcode instead of location_id)
...
DROP TRIGGER IF EXISTS update_query_timestamp ON query;
CREATE TRIGGER update_query_timestamp
    BEFORE UPDATE ON query
    FOR EACH ROW
    EXECUTE FUNCTION update_timestamp();

DROP TRIGGER IF EXISTS update_property_timestamp ON property;
...
DROP POLICY IF EXISTS "Users manage own queries" ON query;
DROP POLICY IF EXISTS "Properties viewable by everyone" ON property;
DROP POLICY IF EXISTS "Users view own query results" ON query_property;
DROP POLICY IF EXISTS "Users manage own property actions" ON user_property_action;
